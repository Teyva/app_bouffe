...
powershell -Command "$json = Get-Content utilisateurs.json | ConvertFrom-Json; $admin = $json.utilisateurs | Where-Object { $_.role -eq 'admin' }; $admin.password = '%new_password%'; $json | ConvertTo-Json -Depth 10 | Set-Content utilisateurs.json"
